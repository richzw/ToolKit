...
- [Binlog Server守护MySQL数据0丢失](https://mp.weixin.qq.com/s/0Lu7_XVCbMIOJkN9h_0o6g)
- [MySQL 默认 RR改为 RC](https://mp.weixin.qq.com/s/7d4CXFk5OBOZr9F3ULf4Ow)
  - RR 与 RC 的区别
    - RC（Read Committed）：只加行锁，可读到已提交数据，能避免脏读，但仍可能出现不可重复读、幻读。
    - RR（Repeatable Read）：行锁 + Gap/Next-Key 间隙锁，可保证可重复读并尽量消除幻读，但锁粒度更大，冲突、死锁概率高。
  - MySQL 早期为何默认 RR
    - • 最初 binlog 只有 Statement 格式（记录 SQL 原文）。
    - • 在 RC 下，主库两个并发事务可能按一套顺序提交，而从库重放语句时顺序不同，造成数据不一致。
    - • RR 依靠间隙锁把这类并发序列串行化，避免主从不一致，因此被设为默认隔离级别。
  - 现在互联网公司改用 RC 的原因
    - • MySQL 5.1+ 已支持 Row/Mixed binlog，复制按照行变更而非 SQL 文本，不再依赖 RR 的间隙锁来保证一致性。
    - • 取消间隙锁后并发度显著提高，锁冲突、死锁更少，延迟更低，适合高 QPS 的互联网场景。
    - • 对偶尔出现的幻读，可通过乐观锁、唯一索引、显式锁表等业务手段规避。
  - 切换到 RC 需注意
...
- [SQL优化](https://mp.weixin.qq.com/s/9vvlXZ6o3p_r-FcZ-0eZ0w)
  - SQL 优化不是“慢了就加索引”，而是从低成本到高成本，按层次逐步推进：
    - 编码与查询习惯（低成本）：减少无效数据读取（避免 SELECT *）、避免让索引失效（函数、隐式类型转换）、减少扫描次数（条件聚合/窗口函数）、避免深分页等。
...
- “单表 2000 万”解释思路：
  - 不是硬上限，而是基于 页大小、行大小、树高 推导的经验阈值；
  - 当数据量导致 B+Tree 层高上升，单次查询可能多一次 I/O（或更多随机访问），性能更容易出现拐点。
- [自增id超过int最大值](https://mp.weixin.qq.com/s/E47LrmRO8Z_2FBHZyoIs2A)
