...
- Iceberg 的真正优势
  ...
  - [Iceberg：湖仓一体的终极寻址设计，分层元数据+统计剪枝，跳过95%的无效文件](https://mp.weixin.qq.com/s/JGC2qDJvNqzyeZMpF97AuQ)
    ...
    - Iceberg的核心创新，是分层元数据+文件级统计信息
      - 在读取数据前，先通过元数据算出「哪些文件需要读、哪些文件可以直接跳过」，从根源减少无效IO。
- [表格格式” vs. “文件（存储）格式]
...
- [存储、数仓、指标设计](https://mp.weixin.qq.com/s/1K5WfHxQT7kmh4YzRldwrA)
  - 数据架构的演进，本质是在以下维度间权衡：**批流统一、延迟、成本、复杂度、治理能力**
    - MPP 架构（大规模并行处理）
      - 核心思想：**分而治之 + 并行计算** 将任务拆成子任务，分发到多节点并行执行，最后聚合结果
    - Lambda 架构（批流分离）
      - 目标：同时满足**超大数据量的精确批处理**和**流数据低延迟处理**。
      - 三层结构：
        - Batch Layer：批处理（如 Spark）——离线全量+高精度。
        - Speed Layer：速度层（如 Flink/Kafka Streams）——增量+低延迟。
        - Serving Layer：服务层（如 HBase/Druid）——对外提供查询。
    - Kappa 架构（流批一体）
      - 核心理念：**一切皆流（Everything is a stream）** 批处理视作对**历史有界流的处理**。
      - 技术实现：
        - 所有数据写入**中心化日志系统**（如 Kafka）。
        - 统一使用流处理引擎（如 Flink）进行计算。
      - 优点：
        - **逻辑统一，架构简化**：仅维护一套代码/一套引擎。
      - 挑战：
        - 对消息队列的**长期存储能力**要求高（海量历史保留）。
        - 流引擎需要强大的**重放 / Reprocessing 能力**。
    - Lakehouse 架构（湖仓一体）
      - 目标：融合**数据湖的低成本 + 数据仓库的管理能力与性能**。
      - 技术基础：开放表格式（Table Format）
        - Apache Iceberg
        - Delta Lake
        - Apache Hudi
      - 关键能力：
  ...
  - 实现 Exactly-once 的关键技术：
    - 1. **分布式快照 / 状态检查点**
      - 典型：Flink 使用 **Barrier + 状态快照**（Chandy-Lamport 算法思想）。
      - 周期性为所有算子做一致性快照，失败后从最近快照恢复。
    - 2. **幂等性写入**
      - 核心：**同一操作重复执行，结果与执行一次一致**。
      - 实现方式：
        - RDBMS（主键约束、唯一索引）
        - NoSQL（版本号 / 条件更新）
        - Kafka（基于 Producer ID 和序列号的幂等写入）
        - 分布式存储（带版本/条件写入）
    - 3. **两阶段提交（2PC）**
      - 与外部系统（Kafka、数据库等）之间保证跨系统的事务原子性。
      - 第一阶段预提交，第二阶段提交或回滚。
  - LSM-Tree
   - 写路径：
     - 写入先到内存 MemTable（通常跳表实现），刷盘形成不可变 SSTable。
     - 后台 Compaction：将多个 SSTable 合并重写。
   - 优点：
     - 大部分磁盘写为顺序写 → 极高写吞吐。
     - 更高压缩率。
   - 缺点：
     - 读放大：读时需查多个 SSTable + 索引。
