...
- [秒杀系统架构](https://mp.weixin.qq.com/s/lpmVfXm_qADDt3wtYwhPTA)
  ...
  - [库存合并扣减：一种基于分布式缓存的强一致性热点库存扣减方案](https://mp.weixin.qq.com/s/_ezTVydFszZnc0ZN-JEtlQ)
    - 传统 Redis 分桶扣减方案的收益与问题
      - 少卖不可避免：Redis 超时/失败时应用层无法确认是否成功，为防超卖倾向按失败处理，导致少卖。
      - 模型受限：仅适合“库存记录是单一数值”的模型，不易支持更复杂的 sq-/wq+ 等联动模型
    - 合并扣减总体思路
      - Redis 只做“扣减计数（防超卖）”，不作为最终事实库存。
      - 最终是否扣减成功以 DB 的扣减明细（单据）为准。Redis 扣减成功后写入 DB 扣减明细；明细写成功才算业务扣减成功
      - 热点流量的真实 DB 库存变更延迟合并提交（例如延迟 1s 扫描明细汇总后一次更新 DB）；DB 从“每笔订单一次 update”变成“周期性批量 update”，大幅降低热点行写压力
  - [MySQL内核秒杀能力](https://mp.weixin.qq.com/s/Q-tap7xI3Y5diTMEt5hMyw)
    - 方案演进对比：
      - 社区 MySQL（热点行 update 串行）- 热点行 update 在 InnoDB 下会因行锁竞争导致吞吐随并发上升而下降
      - V1「排队秒杀」（热点线程排队，曾提升 10x）
        - 对热点 SQL 做“热点线程排队”（本质是把竞争从 InnoDB 行锁竞争，提前搬到更可控的队列/调度点）
      - V2「合并秒杀」（把多个事务的热点更新合并提交，再提升 5x）
        - 将多个事务的热点更新合并，由 Leader 汇总后一次性写入存储引擎提交；Follower 在缓存中完成扣减合并
        - Leader 预读取库存行数据，并写入一个“全局缓存”
        - 多个 Follower 从全局缓存读取、在缓存中扣减并回写全局缓存（完成合并）
        - Leader 收集一段时间后，把合并后的最终值一次性写入存储引擎并提交
    - 合并秒杀核心机制：Leader/Follower + 全局缓存（跨线程共享）
    - 深度优化点：行锁流水线化（phase1/phase2 重叠）、Binlog 组提交、Crash Recovery 兼容
...
- [Everything I know about good system design](https://www.seangoedecke.com/good-system-design/)
  - 好的系统设计往往“毫不起眼”。
    - • 运行稳定、少出问题、让人觉得“比预想容易”就是好设计的标志。
    - • 一开始就堆砌复杂技术（分布式一致性、CQRS 等）多半是缺乏设计或过度设计。
  - 状态是系统设计的难点，应尽量减少有状态组件。
    - • 理想模式：一个服务负责读写数据库，其余服务保持无状态，通过 API 或事件与之通信。
    - • 有状态组件出问题很难自动修复，无状态服务则可随时重启恢复。
  - 数据库往往是瓶颈，也是重中之重。
    - • 模式设计既要足够灵活又要保持可读性；常见查询字段必须建索引。
    - • 让数据库干擅长的事：用 JOIN、避免在业务循环里隐式多次查询。
    - • 只写节点压力最大，读尽量走只读副本；防止写高峰、长事务拖垮整库。
  - 区分“快路径”和“慢路径”。
    - • 用户交互必须百毫秒级返回；耗时操作拆出去做后台任务。
    - • 后台任务的常规实现：Redis(或类似) 队列 + Worker；超长期任务可落库定时扫描。
  - 缓存要慎用。
    - • 缓存本质也是状态，易脏、易错；先优化数据库/算法，最后才考虑缓存。
    - • 对超大、久存的缓存可用定时任务 + 对象存储(S3 等) 持久化。
  - 事件总线（Kafka 一类）适合“这件事发生了”且发送方不关心结果的场景；别滥用，能直接调用 API 就不用事件。
  - 数据分发：
    - • 少量消费者用 push，多量客户端常用 pull；推拉各有成本，视需求选择。
  - 关注系统“热路径”。
    - • 决定成败/流量最大的那几条链路可选方案少、风险大，应投入最多设计精力。
  - 可观测性与容错。
    - • 错误分支要详细日志；监控 CPU/内存、队列长度、平均/ p95 / p99 延迟。
    - • 重试需带熔断器；涉及写操作用幂等键。
    - • 明确组件故障时是 fail-open 还是 fail-closed（如限流宜开、鉴权必须关）。
- [Availability Models](https://www.thecoder.cafe/p/availability-models)
  - 可用性应同时考虑延迟
    • CAP 只要求“返回非错误响应”。
    • PACELC 指出：在无分区时仍需在“低延迟”与“一致性”间权衡；因此延迟是可用性的组成部分。
  - Antithesis / Jepsen 提出的三种可用性模型 用它们来精确定义一次请求在故障场景下是否仍应成功）
    - a) Majority availability（多数可用）
      - 当大多数非故障节点还能互联时，这部分节点可继续执行操作。
      - 常见于需要强一致性的系统（多数写、Leader 选举）。Megastore 把此视为“高可用”。
    - b) Total availability（完全可用）
      - 任一非故障节点都能独立完成操作，即便无法与其他节点通信。
      - 追求极低延迟，但放弃强一致性；Dynamo 以此定义“高可用”。
    - c) Sticky availability（粘性可用）
      - 只要客户端一直连着某个可以反映其历史操作的副本，就能最终得到响应，哪怕分区无限持续。
      - 对于全量复制，可让客户端始终命中同一节点；对分片或局部复制，需要客户端/中间件保持对“一致逻辑副本”的粘性路由，甚至本地缓存。
- [How Uber Serves over 150 Million Reads per Second from Integrated Cache with Stronger Consistency Guarantees](https://www.uber.com/en-HK/blog/how-uber-serves-over-150-million-reads/)
  - CacheFront的规模扩大，Uber面临了提高缓存命中率和提供更强一致性保证的需求。之前依赖TTL和CDC进行缓存失效处理的方法导致了一些一致性问题
    - Compare缓存模式：在读取数据时，查询引擎会比较缓存中的数据版本与存储引擎中的版本。如果缓存数据过时，则从存储引擎获取最新数据，并更新缓存。
    - 增强的缓存失效机制：通过Flux服务，实时监控数据变更，并在检测到更新时，立即使相关缓存条目失效或更新，从而减少提供过时数据的可能性。
    - 所有删除改为软删除（墓碑），后续异步 GC。
...
- [开源限流方案，抛弃redis](https://www.databricks.com/blog/high-performance-ratelimiting-databricks)
  ...
  - 两个关键设计来重构整个系统：内存分片和客户端批量报告。
    - 内存分片：分而治之
      ...
      - 通过引入 Dicer 自动分片技术，相同的限流键总是被路由到同一个节点
    - 批量报告：化零为整
      ```
       1. 客户端默认放行请求（乐观限流）
       2. 本地记录放行和拒绝的数量
...
- [Heartbeats in Distributed Systems](https://arpitbhayani.me/blogs/heartbeats-in-distributed-systems)
  - 心跳间隔（Heartbeat Interval）
    - 大多数系统使用 1-10 秒的间隔
    - 具体取决于应用需求和网络特性
  - 超时/故障阈值（Timeout/Failure Threshold）
    - 典型的经验法则：超时时间应至少是心跳间隔的 2-3 倍
    - 允许丢失一些心跳消息后再判定故障
  - 网络考虑因素
    - 许多系统遵循的规则：超时时间应至少是往返时间（RTT）的 10 倍
  - 实际系统常同时使用 Push 与 Pull：
    - 节点主动发送心跳
    - 监控系统对关键节点再额外轮询
    - 通过冗余提高可靠性
  - 集群规模增大时，中心化监控会成为瓶颈和单点故障。
    - Gossip 协议将失败检测责任分散到所有节点：
    - 每个节点维护本地成员列表，包含所有节点的心跳计数等信息。
    - 周期性随机选取部分节点，交换自己的成员列表。
    - 收到列表时进行 merge，保留每个节点的最新信息。
  - Phi φ 累积故障检测算法
    -  核心特点：
      - 不是简单的布尔值判断（系统正常/故障）
      - 输出连续尺度的怀疑值（suspicion level）
      - 怀疑值越高，系统故障的可能性越大
    - 滑动窗口机制
      - 维护一个采样滑动窗口，保存过去心跳的到达时间
      - 当新心跳到达时，其到达时间被存储到窗口中
      - 最旧的心跳数据被删除
    - 统计模型
...
- [Feed 流系统架构](https://mp.weixin.qq.com/s/53Aqhk0mnT9ru1wmWUZ7jA)
  - Feed 流本质：对“社交关系图 + 海量内容”的实时个性化查询。
    - **读放大**：一次请求需聚合所有关注对象内容，读复杂度随关注数线性增长。
    - **写放大**：写扩散（fan-out on write）下，一条内容要写入所有粉丝时间线，大 V 引发写爆。
    - **实时性**：端到端秒级甚至百毫秒级。
  - 拉取模式（Pull
    - 纯 Pull 适用于早期/小规模产品，不适合大规模社交网络
    - 缺点与教训
      - **读放大严重**：复杂 JOIN + 排序，时间复杂度 O(n_followees)。
      - 水平扩展能力差，**数据库成为单点瓶颈**。
  - 写扩散模式（Fan-out on Write
    - 核心思想
      - **写入时预计算时间线**：用户发帖时，将内容异步“扇出”到所有粉丝的时间线存储（Inbox）。
      - 读请求只需 O(1) 从 KV/缓存中取该用户时间线。
    - 使用 **Redis / Cassandra / 自研 KV**：
      - Redis Sorted Set / List 存储每个用户的 tweet_id 列表或有序集合。
      - Cassandra 用于大规模持久化。
    - 写入异步化：
      - 发帖 → 写内容存储 → 投递到 MQ（RabbitMQ / Kafka / 自研队列）→ Worker 扇出写入粉丝时间线。
    - 优点
      - **读性能极佳**：
        - 时间复杂度近似 O(1)，P95 读延迟 < 10ms。
        - 高缓存命中率（>95%），非常适合缓存预热。
      - 大幅减轻数据库查询压力，数据库更专注于元数据、关系等。
    - 致命缺陷：写放大  纯推模式解决了读瓶颈，但在存在大 V、超级节点时写入成本不可接受。
  - 混合模式（Hybrid Push/Pull
    - 按用户类型差异化处理
      - **普通用户（绝大多数）**：继续使用写扩散（Push）。
      - **大 V / 超级节点**：改用读扩散（Pull，不预写入全部粉丝），或精细化策略
    - 混合模式成为 **事实标准**，解决了 push-only 与 pull-only 各自的极端问题
  - 分布式队列与异步流水线
    - 通过 **分布式消息队列（Kafka 等）** 将流量尖峰转化为可控流水线：
      - 解耦“写 API”与“fan-out 计算”。
      - 利用批处理、背压、重试保证稳定性与可靠性。
    - Kafka 等日志系统成为 **Feed 流、事件流的统一底座**
  - Summary
    - 演进脉络
      1. **拉取模式**：简单易用 → 死于读放大与 DB 瓶颈。
      2. **写扩散**：读极快 → 死于大 V 写放大。
      3. **混合模式**：按用户类别平衡读/写成本 → 成为标准模式。
      4. **分布式队列**：将写入峰值平滑为可控流量 → 稳定性大幅提升。
      5. **机器学习排序**：从时间线到个性化推荐 → engagement/收入显著提升。
      6. **全球化优化**：多活、多 DC → 全球一致低延迟和高可用。
    - 现代 Feed 流设计核心原则
      - **分层解耦**：写入、扇出、存储、排序、读取严格分层。
      - **异步优先**：写入与计算尽量通过 MQ 异步化。
      - **混合推拉**：用写扩散优化绝大多数用户读体验，用读扩散保护系统免于写爆。
      - **多级缓存**：热点强缓存、冷数据下沉，命中率做到极致。
      - **机器学习驱动**：排序/推荐与业务目标紧密耦合，持续 A/B 迭代。
      - **全球多活 + AIOps**：为超大规模与高可用提供基础设施级能力。
- 图数据库风控
