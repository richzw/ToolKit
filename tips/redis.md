...
- [Redis 8.0 vs. Valkey ](https://www.dragonflydb.io/blog/redis-8-0-vs-valkey-8-1-a-technical-comparison)
  - https://mp.weixin.qq.com/s/1RWQKZxJf9avYLB5puPERg
  - 异步 IO 线程
    - Redis6.0 多线程 IO 的缺陷：主线程在执行命令时，IO 线程会空闲；IO 线程仅负责少量读写操作，主线程仍要阻塞等待，负载较重。
    - Valkey8.0 新的异步 IO 线程模型：
      - 通过环形队列给每个 IO 线程派发读写任务，IO 线程可并行解析命令、写回结果，减少主线程等待。
      - 支持动态调整 IO 线程数量（上限 15），根据客户端读写事件负载进行增减。
      - 除读写网络数据外，还把事件轮询(epoll_wait)、对象释放、命令查找等耗时操作卸载到 IO 线程执行，大幅减轻主线程负担。
  - 数据预取 (Prefetch) 与内存访问分摊 (MAA)
    - 随着 CPU 与内存速度差距增大，传统的串行查找 key 过程易出现“内存成为瓶颈”的问题。
    - Valkey8.0 通过两项优化提高哈希表查找效率：
      - 数据预取：在执行命令前，主动使用 __builtin_prefetch() 将可能访问的数据（命令参数、key、value 等）提前加载至 CPU 缓存，减小等待延迟。
      - 内存访问分摊 (MAA)：将多个 key 的处理交叉进行，每次只完成一步预取操作就切换到下一个 key，减少因等待单个 key 内存读取而阻塞的时间，使所有 key 的访问在缓存中并行加载。
    - 结合批量处理（一次最多可批 16 条命令），提升了哈希表查找和命令执行效率，社区测试单节点 QPS 可达到 120 万。
...
- [Redis 8.0 RDB Channel Replication ](https://mp.weixin.qq.com/s/LBLGucHFgtdLBdDBL2-GQg)
  - Redis 8.0 引入了基于 RDB channel 的全量同步，避免主库同时承担缓存写命令和 CoW 的内存压力，提高全量同步的成功率
  - Background
