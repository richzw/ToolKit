...
- [RocksDB 高性能](https://mp.weixin.qq.com/s/40JnearOCVDaGbU7WpxB1g)
  - RocksDB 是内嵌的数据库，所以它是没有独立进程的
  - RocksDB 基础的组件是MemTable、SSTable和 预写日志（WAL）日志
...
- Kafka性能优化
  - 趋势有相关的指标有 RequestQueueSize大小、日志刷盘持续时长、BROKER生产请求数
  - 刷盘时间指标验证 Kafka 的刷盘时间指的是将数据从内存刷新到持久存储设备（如硬盘）所需的时间
...
- 消息队列积压咋办
  ...
  - 监控与告警
- [MQ的时候，怎么确保消息100%](https://mp.weixin.qq.com/s/mJcrS-nOv8aui43s4WSiqw)
  - Lost msg
    - 生产	网络抖动、Broker 宕机，导致消息没送达
      - ACK 机制 
      - 本地消息表（解决“业务 + 发送”原子性）
    - 存储	Broker 未持久化、主从未同步完就下线
...
- 高并发下如何保证消息顺序消费
  - 消息的核心概念
    - 顺序的定义（以Broker接收顺序为准）
    - Kafka的消息存储机制（Partition级别的有序性）
    - 全局顺序与局部顺序的区别
    - 跨Topic消息有序性的处理思路
  - 顺序消费解决方案及演进
    - 方案一：单Topic单分区（最基础，有性能瓶颈）
    - 方案二：单分区 + 异步消费（引入内存队列，有数据丢失风险）
    - 方案三：多分区局部有序（主流方案，按业务键Hash）
    - 多分区方案的生产级痛点与优化
      - 痛点1：数据倾斜 -> 优化：一致性哈希、虚拟槽映射
      - 痛点2：扩容引发顺序错乱 -> 优化：新分区消费者“冷静期/静默期”延迟消费
