...
- [Enterprise Agents Have a Reliability Problem](https://www.dbreunig.com/2025/12/06/the-state-of-agents.html)
  - 现成 AI 工具（3rd party）在企业内被广泛使用且被认为有价值（如 ChatGPT、Copilot、Gemini 等）。
  - 内部自建的 AI 试点项目（AI pilots）大多无法真正被员工采用。
  - 能从试点阶段走向生产环境的企业 Agent 非常少：根据 McKinsey 的调研，少于 10% 的受访企业有这样“真正上线”的内部 Agent。
  - 为达成可靠性，开发者被迫构建更简单的 Agent：缩短任务链路、减少自主性、保持人类在环。可靠性成为主导性约束。
  - 可靠性是最突出、最核心的开发难题： 难在两件事：
    - 如何确保 Agent 的正确性（correctness）；
    - 如何评估正确性（evaluation）。
  - 为了对抗可靠性问题，团队采用了高度保守的工程策略：
    - 绝大多数直接使用现成大模型（off-the-shelf LLM），
      - 不做微调（no fine-tuning），
      - 主要靠手工调 prompt 来控制行为。
    - Agent 运行步骤极短：
      - 68% 的 Agent 在执行少于 10 步后就需要人类介入。
    - 人类在环（human-in-the-loop）是绝对主流：
      - 92.5% 的生产级 Agent 将输出直接交给“人”而不是其他软件/Agent。
    - 总结：组织会有意地限制 Agent 的自主性，以维持可靠性。
  - 短期内成功的团队会采用“先小后大、逐步扩张”的策略：
...
- [Dynamic context discovery](https://cursor.com/blog/dynamic-context-discovery)
  - 随着模型变得越来越聪明，预先塞太多信息反而帮倒忙。一来浪费 token（上下文窗口是有限的），二来信息太杂可能干扰模型判断
  - （Dynamic Context Discovery）模式：别急着把信息塞给模型，而是让模型在需要的时候自己去找
  - 一：长输出变成文件
    - 把长输出写成文件，然后给 AI 一个读文件的能力。AI 可以先用 tail 命令看看结尾，觉得需要再往前读。
  - 二：聊天历史变成可查档案
    - 当对话太长，超过上下文窗口限制时，Cursor 会触发一个"总结"步骤，把之前的内容压缩成摘要，给 AI 一个"新的起点"
    - 压缩是有损的。重要细节可能在总结过程中丢失，导致 AI"失忆"
    - Cursor 的办法是把完整的聊天记录存成文件。AI 拿到的是摘要，但如果它意识到"这里好像漏了什么"，可以自己去翻原始记录找回来
  - 三：按需加载技能
    - Agent Skills"的扩展机制，本质上是告诉 AI 怎么处理特定领域任务的说明书。这些说明书可以有很多，但没必要每次都全部加载
    - 只在系统提示里放一个"目录"——技能的名字和简短描述。AI 真正需要某个技能时，再用搜索工具把完整说明拉进来。
  - 四：MCP 工具的瘦身术
    - 只在提示词里放工具的名字，完整描述同步到一个文件夹。AI 需要用某个工具时，再去查具体怎么用
