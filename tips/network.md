...
- [Tail Loss Probe (TLP)](https://mp.weixin.qq.com/s/PQ7C26aiUAcx7nmFbxPycw)
  - TCP Tail Loss Probe (TLP) 是一种用于改进 TCP 在发生尾部丢包时的性能的机制，简单来说，就是在检测到可能发生尾部丢包时，也就是当连接一段时间（称为 Probe Timeout，即 PTO，且 PTO ≤ RTO）未收到 ACK 时，强制发送一个探测包
...
- TCP RACK
  ...
  - 其核心思想是通过比较数据包的发送时间戳和确认到达时间来智能判断丢包：
    - 若一个较早发送的数据包在较晚发送的数据包已被确认后，其等待时间超过动态计算的网络最大容忍延迟窗口（reordering window），则判定该数据包丢失，这种方法突破传统依赖 Dup ACK 数量的限制，直接利用时间维度信息进行决策。
  - RACK 的核心优势在于更快速、更精准的丢包响应，尤其在乱序网络环境中表现突出
    - 它能显著减少对网络乱序的误判，有效解决尾部丢包问题，并降低不必要的重传。与 SACK 协同工作时，可进一步提升检测效率，优化带宽利用率。
- TCP DSACK
  - TCP DSACK（Duplicate Selective Acknowledgment）是 TCP 协议中 SACK（选择性确认）机制的扩展，允许接收方在确认数据时，不仅通知发送方已成功接收的非连续数据块，还能明确指示哪些数据段被重复接收。
  - 通过这种方式，发送方能够区分是原始数据包丢失还是仅仅延迟到达（从而触发了不必要的重传），进而更精确地判断网络状况（如是否出现冗余重传或乱序），并优化重传策略，提升网络传输效率。
  - 接收方通过在 SACK 选项的第一个块中特意报告重复接收的数据段范围（即 DSACK 块），向发送方明确指示发生了数据重复
    - tcp 参数为 net.ipv4.tcp_dsack ，默认值为 1
- Port Forwarding
  - ![img.png](network_port_forwarding.png)
- TCP FRTO（Forward RTO-Recovery）
  - 一种用于优化 TCP 性能的机制，其核心原理在于区分虚假超时与真实丢包。一种在超时重传（RTO）发生后，用来判定“超时是否是虚假的（spurious）”并尽量避免不必要拥塞退避的算法
    - Linux 系统下相关的 tcp 参数为 net.ipv4.tcp_frto ，默认值为 2
    - 当 RTO 触发时，传统 TCP 会把拥塞窗口骤降并进入慢启动；
    - F-RTO 会先做一次小心的“探测”，如果发现实际上并没有真正丢包（只是临时时延/重排序等），就撤销过度退避，继续按原来的速率前进，从而提升性能。
  - 在无线、移动网络或出现路由/排队抖动的场景，RTT 短时飙升或数据包重排序容易让定时器误判为丢包，触发 RTO；传统做法会把 cwnd 降得很低，吞吐显著受损。F‑RTO 旨在识别这类“虚假超时”，避免无谓减速。
- 在高 RTT 场景（跨洋主机）跑 ping -f，一段时间后 Ctrl+C；随后抓包或再开一个终端观察，会看到程序结束后仍有 Echo Reply 抵达，这些回包未被计入统计
  - 避免手动 Ctrl+C，用固定次数/间隔让程序自己收尾：
    - 例：ping -c 200 -i 0.2
    - 给每包设置等待超时：-W 2（单位秒，按需要调整）
  - 将发送间隔设得不小于典型 RTT 的 1–2 倍，减少在途包数量，降低“尾包未归”带来的偏差。
  - 需要高压时尽量不用 -f；若必须，用 -c 或 -w 让它按固定包数/时长结束，而不是人工中断（iputils 的 ping 没有“drain 尾包”的机制，Ctrl+C 结束就不再等回包）。
  - 用更合适的工具做批量/高并发探测或链路质量评估：
    - fping：fping -c 200 -p 20 -t 200
    - mtr：mtr -rwc 200 （同时看丢包与抖动）
    - nping（nmap）：可控速率/并发与“收尾”等待
- TCP 异常重复数据
  - Linux TCP 按“先到先得” 保留同一序号范围第一份数据。
    - 重叠发生时：
      - 若 后到达段的“前部” 与已缓存数据重叠 → 丢弃后到达段的重叠部分
      - 若 后到达段的“尾部” 与已缓存数据重叠 → 丢弃已有段的重叠部分
  - 内核通过 DSACK（Duplicate SACK） 把“被丢弃的重复数据段范围”通知发送端，格式表现为 第二个 SACK 块完全覆盖第一个块 
- Silly Window Syndrome 
  - Silly Window Syndrome 是指通信双方（发送端和接收端）以一种“低效”的方式工作，导致网络传输了大量有效载荷很小（比如只有1字节）的 TCP 报文段
  - 接收端通过 Clark 算法避免问题，
    - 其核心是不通告小窗口：当回复 ACK 的时候，如果可用的接收缓存过小则直接通告零窗口；
    - 当应用层读取数据，只有空闲接收缓存大于一定值的时候，会根据空闲缓存计算出一个新的 Window Size，如果这个新的 Window Size 大于等于两倍的当前接收窗口，才会立即触发窗口更新
  - 发送端则通过 Nagle 算法来避免，
    - 其核心是合并小数据：如果之前发送的数据还未被确认，发送端会缓存后续传来的一些小数据，等待它们合并成一个更大的报文或在收到ACK后再发送，以此减少网络中小包的数量。
- [KCP 与 SD-WAN](https://blog.mfwt.top/index.php/archives/912/)
