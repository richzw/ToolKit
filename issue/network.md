...
- TCP连接“假死”
  - 新主节点 (10.0.10.227) 持续向旧主节点原 IP (10.0.10.129:5022) 发送数据，但收不到 ACK。
    - TCP 机制：触发 RTO（超时重传），重传间隔呈指数级退避（0.4s → 0.9s → 1.7s ... 106.5s）。
    - 系统默认行为：Linux 默认配置下会重传 15 次，总耗时约 15 分钟才会彻底放弃。
  - 主动解法（应用层）：为 MSSQL Addon 引入 Graceful Shutdown Patch，接收到停止信号时调用 close() 释放 TCP 连接，发送 FIN/RST。
  - 被动防御（系统层）：修改内核参数 net.ipv4.tcp_retries2=8。通过 sysctl 或 initContainer 注入，强制 TCP 层在约 25 秒后断开连接并向上层抛出 ETIMEDOUT 异常。
