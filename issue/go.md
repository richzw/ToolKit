  ...
  - https://mp.weixin.qq.com/s/PMmj-jloOUGpCJYlD6IfQQ
    ...
    - 传统 Go GC
      ...
      - 瓶颈
        ...
        - Summary
          - Poor cache locality; - 顺序扫描页，在内存页连续处理对象，最大化命中率
          - Numa Challenge;  - 本地化处理，页扫描
          - 内存带宽限制 - 批量处理， 页级别工作队列更小，线程竞争少
          - 新硬件 - 拥抱SIMD
    - Green Tea : Work with pages, not objects
      ...
      - Details
        - Scan Whole page
        - Track pages - work list存放待处理的内存页，而不是对象； 工作列表的顺序从 LIFO（后进先出，类似栈）改为 FIFO（先进先出，类似队列）。
        - Localize metadata 每个对象内部追踪对象的标记状态
...
- [Go Trace 工具揭示的性能真相](https://mp.weixin.qq.com/s/7ZEBgBBqGXat9eOPZKEOSw)
  ...
  - 一个“看似更优雅的并发设计（goroutine pool）反而更慢”的案例，并用 Go trace 工具逐层定位原因：性能下降并非因为 CPU 没吃满，而是 GC 的 pacing（调速）过度频繁介入
    - 工作池模式 (Worker Pool) 实际上比“扇出模式 (Fan-out)”更慢，主要原因
    - 垃圾回收 (GC) 过度调优与频繁触发
      - 当程序限制 Goroutine 的数量（工作池模式）时，Go 运行时为了追求极致的内存效率，会将堆内存（Heap）维持在一个非常低的水位（约 13 MiB）
    - GC 耗时占比过高：通过 Trace 工具分析发现，工作池模式下程序有 74% 的时间都花在了 GC 上
    - 运行时调度器的特性：Go 的调度器和垃圾回收器在设计上已经针对高并发的“扇出”模式
      - 开发者有时试图通过工作池手动控制并发，反而可能违背了运行时的优化逻辑
